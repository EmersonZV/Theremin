...
int echo = 5;     // Pin para echo
int trigger = 4;  // Pin para trigger   d:\SANTO_TOMAS\IOT\Avance18112024\frecuencias.h
int echo_2 = 6;
int trigger_2 = 7;
int speaker = 9;  // Pin para el buzzer
unsigned long tiempoRespuesta;
unsigned long distancia, distancia_2;
int limiteInferior = 5;
int limiteSuperior = 45;
int cantidadNotas = 8;
int cantidadNotas_2 = 13;
...
int notas_2[]={NOTE_C4,NOTE_D4,NOTE_E4,NOTE_F4,NOTE_G4,NOTE_A4,NOTE_B4,NOTE_C5}; // Escala Do mayor
//int notas_2[]={NOTE_C4,NOTE_CS4,NOTE_D4,NOTE_DS4,NOTE_E4,NOTE_F4,NOTE_FS4,NOTE_G4,NOTE_GS4,NOTE_A4,NOTE_AS4,NOTE_B4,NOTE_C5}; //Escala cromatica
...
void loop() {
  ...
  digitalWrite(trigger_2, HIGH);  // Enviamos pulso de 10 microsegundos
  delayMicroseconds(10);
  digitalWrite(trigger_2, LOW);
  tiempoRespuesta = pulseIn(echo_2, HIGH);  // Y esperamos pulso de vuelta
  distancia_2 = tiempoRespuesta / 58;       // Calculo de distancia en cm
  ...
  if (distancia_2 >= limiteInferior && distancia_2 <= limiteSuperior) {
    int x = map(distancia_2, limiteInferior, limiteSuperior, 0, ((cantidadNotas_2 - 1) * 10));
    tone(speaker, notas_2[x / 10]);
    delay(100);
  } else {
    noTone(9);
  }
